...
import com.controller.java.ticketdetails.WeeklyDailyDetails;
...
public class CRUD_Tickets {    
    ...
    String insertStmt = "INSERT INTO public.weekly_daily(id, start_date, end_date, ticket_type, location_name) VALUES (?, ?, ?, ?, ?)";
    ...
    String oneway_Stmt = "SELECT OW.departure_date, OW.direction, TI.location_name, TI.morning_pickuptime, TI.afternoon_pickuptime "
                       ...
                       + "WHERE OW.id = ? "
                       + "ORDER BY OW.departure_date DESC, OW.direction ASC";
    ...
    try (PreparedStatement stmt = conn.prepareStatement(oneway_Stmt)){
        ...
        while(OW_rSet.next()){
            ...
            ticket_details.add(new OneWayDetails(id, departure_date, location, direction, pickuptime));
        }
    }
    catch(SQLException sqle){
        throw new TicketSelectionException(sqle.getMessage());
    ...
    public static void delete_tickets(Connection conn, String id) throws TicketDeletionException{
        String delete_weekly_daily_Stmt = "DELETE FROM public.weekly_daily WHERE (ID = ? AND end_date < CURRENT_DATE)";
